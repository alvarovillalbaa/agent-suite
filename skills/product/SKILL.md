---
name: product
description: "Product management and product strategy skill for shaping PRDs, prioritization, roadmaps, discovery summaries, experiment plans, release criteria, product decision memos, quarterly strategic planning, OKR cascades, product vision documents, competitive landscape analysis, product team design proposals, and high-converting landing pages. Also covers agile execution: user story writing (INVEST, Given-When-Then), acceptance criteria, epic breakdown, sprint planning, velocity tracking, and backlog management. Also covers content strategy: content pillar development, searchable vs. shareable content planning, keyword research by buyer stage, topic cluster mapping, and content prioritization. Also covers marketing psychology: 50+ buyer psychology and persuasion mental models (loss aversion, social proof, anchoring, scarcity, JTBD, BJ Fogg, pricing psychology, growth loops) for conversion optimisation, onboarding, retention, and go-to-market strategy. Also covers UX copy: writing and reviewing interface copy (CTAs, error messages, empty states, confirmation dialogs, tooltips, loading states, onboarding) following Clear-Concise-Consistent-Useful-Human principles. Also covers CRO auditing: diagnosing underperforming marketing pages across seven dimensions (value proposition clarity, headline effectiveness, CTA placement, visual hierarchy, trust signals, objection handling, friction points) and delivering prioritized quick wins, high-impact changes, A/B test ideas, and copy alternatives. Use when the user needs product thinking rather than implementation planning: define the problem, user, value, strategic lens, tradeoffs, success metrics, alignment, and decision rationale before engineering execution. Also use when the user asks to create, improve, or A/B test a landing page, marketing page, homepage, or conversion-optimised web page — generates complete Next.js/React TSX components with Tailwind CSS, proven copy frameworks (PAS, AIDA, BAB), SEO meta, and Core Web Vitals targets. Also use when the user asks about buyer behavior, persuasion techniques, pricing perception, reducing churn, improving activation, or applying psychological principles to marketing. Also use when the user needs to write or audit UI copy, microcopy, or interface text across any product surface. Also use when the user wants to audit, diagnose, or improve an existing marketing page's conversion rate."
metadata:

...
Score each candidate on three dimensions. Promote when total ≥ 6.
...
- **Promote when total ≥ 6**
- Watch when total = 4–5
- Ignore when total ≤ 3
...
- It is short enough to add without exceeding 200 lines
...
Transform the learning before promoting it.

### From descriptive to prescriptive

```
❌ "I noticed the project uses pnpm workspaces. npm install fails because of
    the lock file."
...
```
❌ "When modifying API endpoints in the OpenAPI spec file, you need to
    regenerate the TypeScript client by running the generate command,
    otherwise the types won't match at runtime."

✅ "After editing `openapi.yaml`: run `pnpm run generate:api` to regenerate
    TS client."
```
...
```
❌ "Sometimes you need to restart the dev server after changing environment
    variables."

✅ "Restart dev server after any `.env` change — hot reload doesn't pick up
    env vars."
```
...
Candidates that are not promoted within 30 days become stale. Stale candidates should be re-evaluated — either promote, delete, or reset the clock if still valid.

| State | Age | Action |
|-------|-----|--------|
| Fresh | < 7 days | Review and score |
| Watch | 7–30 days | Confirm recurrence before promoting |
| Stale | 30–90 days | Re-evaluate: still relevant? |
| Expired | > 90 days | Delete unless actively confirmed |

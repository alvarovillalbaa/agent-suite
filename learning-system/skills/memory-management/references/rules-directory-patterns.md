...
Best practices for `.claude/rules/` — the scoped instruction system that loads rules only when relevant files are open.
...
| Team adds a convention for a specific area | New rule file |
...
A rules file without a `paths` field loads every session, exactly like CLAUDE.md. Use sparingly. Common candidates:

- `security.md` — universal constraints that apply everywhere
- `code-style.md` — project-wide formatting conventions
- `architecture.md` — architectural decisions that apply to all code

## Capacity Impact

| Type | Load frequency | CLAUDE.md line budget used |
|------|---------------|--------------------------|
| CLAUDE.md entry | Every session | Yes |
| Unscoped rules file | Every session | No (separate file) |
| Scoped rules file | Only when paths match | No (conditional) |

Moving project-wide rules from CLAUDE.md to a rules file frees CLAUDE.md budget even when the rules still load every session.
